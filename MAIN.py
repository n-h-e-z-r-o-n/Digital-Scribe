...
def chat(widget):
    ...
    def add(widget):
        new_entity = tk.Frame(widget, bg=bg_color, borderwidth=2, border=1, height=90, width=int(screen_width*0.9747*0.21 -5))
        new_entity.pack(side=tk.TOP, fill=tk.X)

        tk.Entry(new_entity, bg=bg_color, borderwidth=0, border=1).place(relx=0.01, rely=0, relwidth=0.5, relheight=)
        tk.Entry(new_entity, bg=bg_color, borderwidth=0, border=1).place(relx=0.52, rely=0, relwidth=0.2, relheight=1)
        tk.Button(new_entity, bg=bg_color, text="X", borderwidth=0, border=0, font=("Bauhaus 93", 11) ).place(relx=0.95, rely=0, relwidth=0.05, relheight=1)
